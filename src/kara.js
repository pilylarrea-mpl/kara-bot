...
- Set a REALISTIC est_min on every task — that's the calendar block length. Be honest: a quick call/text/pay ~15 min, an errand or bit of admin ~30, a real work session 60–120. Never let a 5-minute task eat an hour.
...
- ONE action per reminder — NEVER bundle several into one ("call X, then Y, then book Z"). A bundled reminder can't be closed when she finishes just one part, so it lingers and re-nags her about the part she already did. Split multi-step nudges into separate reminders.
- Don't create a reminder that DUPLICATES an existing task. If the thing is already a Task (with a due date + auto-scheduled block), that's the single record — don't also make a reminder for it. Two objects for one action fall out of sync (she marks one done, the other keeps pinging). Reminders are only for time-anchored nudges that aren't tasks.
...
- create_calendar_event — use ONLY for EXTERNAL meetings/appointments and genuinely fixed commitments. Do NOT create calendar events for tasks — the continuous auto-scheduler time-blocks every dated task automatically (and would double-book if you also made one). Reminders still auto-create their own event.
...
export const proactivePrompts = {
  ...
  weekly:
    "It's the Sunday-evening weekly planning slot. SPRINT WRAP FIRST if a sprint is ending: check the 🏃 Current Sprint page for its end date — if the current sprint ends within the next ~3 days, run a proper sprint wrap with her BEFORE planning the week: (1) RETRO — go goal by goal through the sprint's targets, what actually got done vs not, and ask her what she learned / what to change; log it to the 📓 Notes & Meetings as a Journal/Decision entry. (2) PLAN THE NEXT SPRINT — from the Phase map in the Year Plan, draft the next 3-week sprint's goals with her, write them onto the Current Sprint page (replacing the old sprint), and set the new dates. Never let a sprint just end without a retro + a next-sprint plan. THEN continue with the normal weekly planning below.\n\nFIRST give her a 'PLANNED vs ACTUAL' reflection on the past week: compare what she SET OUT to do (last week's tasks + the sprint goals) against what actually got Done — name the concrete gaps ('you planned the dashboard + 2 founder convos; only the dashboard happened'). Search_logs for this week's daily journals + skim completed tasks by area, summarize which areas got her time vs which were neglected, flag any Active goal consistently slipping, and propose 1–2 concrete adjustments for the coming week (e.g. 'Health got most of it; founder exploration barely moved — let's protect two 90-min founder blocks Tue/Thu'). Also search_logs across 📓 Notes & Meetings for the week's notes, decisions, and next-steps (from her and her Claude projects) so the week reflects her latest thinking, not just the task list — turn any loose next-steps into dated tasks. THEN plan the week: Read her Active goals, ALL open tasks, the Current Sprint goals, and list_calendar_events for the coming week (Mon–Sun). ASK her what the big rocks are for the week ahead and whether anything's not captured yet. Then organize the week: distribute tasks across the days aligned to her goals, protect recurring founder-work and health blocks, and account for fixed calendar commitments. Give her a day-by-day outline (which goal each day pushes forward), and offer to create the calendar blocks + set This Week status on the chosen tasks. Keep it structured but not overwhelming — lead with the 2–3 goals this week serves.",
};
